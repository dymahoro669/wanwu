...
2. [技术架构](#2-技术架构)
3. [项目结构](#3-项目结构)
4. [⚠️ 重要：public/ vs dist/ 目录规则](#4-重要public-vs-dist-目录规则)
5. [快速开始](#5-快速开始)
6. [页面系统](#6-页面系统)
7. [设计规范](#7-设计规范)
8. [组件开发](#8-组件开发)
9. [构建与部署](#9-构建与部署)
10. [Android原生集成](#10-android原生集成)
11. [测试](#11-测试)
12. [常见问题](#12-常见问题)
13. [开发经验教训](#13-开发经验教训)
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 1.1.0 | 2026-04-23 | 新增开发经验章节、修正页面架构说明 |
...
### 13.1 ⚠️ public/ vs dist/ 目录规则（最重要）

这是最容易犯的错误，已经因此丢失过代码。

```
wanwu-app/
├── public/                 # ⭐ 源文件目录 = 修改代码的地方
│   ├── 01-登录/
│   ├── 03-首页/
│   └── ...
│
├── dist/                   # ⭐ 构建输出目录 = 自动生成，不要直接修改
│
└── index.html
```

**错误做法**：直接修改 `dist/` 目录的文件
```bash
# ❌ 错误！dist/ 是自动生成的
vim dist/_6/code.html
```

**后果**：每次运行 `npm run build` 会用 `public/` 的内容覆盖 `dist/`，所有修改丢失。

**正确做法**：修改 `public/` 目录，然后构建
```bash
# ✅ 正确
vim public/03-首页/code.html
npm run build          # Vite 把 public/ 复制到 dist/
...
`index.html` **不是页面**，而是**主容器**：

```
index.html (容器)
├── 视频背景 (z-[-2]) - 始终播放
├── 视频遮罩 (z-[-1])
├── 首帧图遮盖 (z-[0]) - 视频加载时显示，加载完成后渐隐
├── page-container (bg-transparent) - 承载加载的页面
└── 底部导航 (z-50)
```

启动画面应该是 `public/10-启动画面/code.html`，通过路由动态加载到 page-container 中。

### 13.3 页面路由配置

页面目录重命名后，必须同步更新 index.html 中的路由映射：

```javascript
const pages = {
    'splash': '10-启动画面/code.html',     // 启动画面
    'login': '01-登录/code.html',          // 登录页
    'fingerprint': '08-指纹识别/code.html', // 指纹识别
    'home': '03-首页/code.html',           // 首页
    'assetDetail': '09-资产详情/code.html', // 资产详情
    'profile': '07-个人中心/code.html',    // 个人中心
    'data': '05-数据中心/code.html'         // 数据中心
};
```

### 13.4 子页面跳转

在子页面中使用 `window.parent.loadPage()` 调用父窗口的路由函数：

```html
<!-- 指纹识别页 -->
<button onclick="window.parent.loadPage('login')">使用工号密码登录</button>
<div onclick="window.parent.loadPage('home')">指纹图标</div>
```

### 13.5 视频背景页面设置

需要使用视频背景的页面，body 必须透明：
...
```html
<div class="absolute inset-0 bg-gradient-to-b from-black/30 via-black/20 to-black/50 z-0 backdrop-blur-[2px]"></div>
```

### 13.6 启动流程

```
1. 加载 index.html → 显示首帧图 + 视频开始播放
2. loadPage('splash') → 加载启动画面到 page-container
3. 视频 canplay → 首帧图遮盖渐隐，露出视频
4. 启动画面文字叠加在视频上（3秒）
5. 3秒后 → loadPage('fingerprint') 进入指纹识别页
```
...
**问题**：所有代码修改被写入 `dist/` 目录而非 `public/` 目录，导致每次 `vite build` 后修改丢失。

**根因**：没有理解 Vite 项目的构建流程，将构建输出目录当作源文件目录使用。

### 2026-04-23 - 逐个修改导致返工

**问题**：修改登录页时每次只改一个问题（字号、字体、颜色等），导致反复修改。

**教训**：修改任何页面之前，必须先完整读取原型 HTML，将所有关键样式列成清单，一次性修改到位。

### 2026-04-23 - 启动页架构理解错误

**问题**：最初将启动画面内嵌在 index.html 中，而非使用独立的页面文件。

**教训**：index.html 是容器，启动画面应该是 `public/10-启动画面/code.html`，通过路由动态加载。
